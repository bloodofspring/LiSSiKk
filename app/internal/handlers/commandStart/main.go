...
func SendGreetingMessage(c tele.Context, args *handlers.Arg) (*handlers.Arg, *e.ErrorInfo) {
	err := c.Reply(fmt.Sprintf("Привет! %s!\n\nЭта штука работает как обычный чат: каждое сообщение отправленное в бота будет доставлено эдмину бота; его ответы будут приходить сюда же.\nПоддерживаются все типы меиа, а также стикеры, видеосообщения, голосовые итд. Также бот поддерживает реплаи (ответы на сообщения)\n\nЗачем это сделано?\nДанный бот призван максимально минимизировать неудобства при общении при этом оставив меня анонимным. Это обеспечивает мою ьезопасность ;)", c.Sender().FirstName))
	if err != nil {
